...
results_2005 = results[results['Benchmark Year'] == 2005].copy()
...
gdp_2005 = gdp_df[['Country Name', '2005']].rename(columns={'2005': 'GDP_PPP_2005'})
merged = results_2005.merge(gdp_2005, left_on='Holdout Country', right_on='Country Name', how='left')


merged['Log_GDP'] = np.log10(merged['GDP_PPP_2005'])
...
plt.title('Relationship Between Economic Wealth and Model Accuracy (2005)', fontsize=14)
plt.xlabel('Log10(GDP per capita, PPP)', fontsize=12)
plt.ylabel('Model Accuracy ($R^2$)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('gdp_vs_accuracy_log2005.png', dpi=300)
plt.show()

missing = merged[merged['GDP_PPP_2005'].isna()]['Holdout Country'].unique()
if len(missing) > 0:
