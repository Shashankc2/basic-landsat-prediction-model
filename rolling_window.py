...
data = pd.read_parquet('outcomes.parquet').merge(
    pd.read_parquet('landsat_embeddings.parquet'),
...
for i in range(len(years) - window_size + 1):
    ...
    train_data = data[data['year'].isin(window_years)]
    
    model = Ridge(alpha=1.0) 
    model.fit(train_data[features], train_data['iwi'])
...
plt.title("Evolution of Economic Predictors (5-Year Rolling Windows)", fontsize=14)
...
plt.savefig('feature_stability_plot.png', dpi=300)
plt.show()