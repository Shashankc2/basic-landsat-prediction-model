...
results_1995 = results[results['Benchmark Year'] == 1995].copy()
...
gdp_1995 = gdp_df[['Country Name', '1995']].rename(columns={'1995': 'GDP_PPP_1995'})
merged = results_1995.merge(gdp_1995, left_on='Holdout Country', right_on='Country Name', how='left')


merged['Log_GDP'] = np.log10(merged['GDP_PPP_1995'])
...
plt.title('Relationship Between Economic Wealth and Model Accuracy (1995)', fontsize=14)
plt.xlabel('Log10(GDP per capita, PPP)', fontsize=12)
plt.ylabel('Model Accuracy ($R^2$)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('gdp_vs_accuracy_log1995.png', dpi=300)
plt.show()

missing = merged[merged['GDP_PPP_1995'].isna()]['Holdout Country'].unique()
if len(missing) > 0:
