...
results_2015 = results[results['Benchmark Year'] == 2015].copy()
...
gdp_2015 = gdp_df[['Country Name', '2015']].rename(columns={'2015': 'GDP_PPP_2015'})
merged = results_2015.merge(gdp_2015, left_on='Holdout Country', right_on='Country Name', how='left')


merged['Log_GDP'] = np.log10(merged['GDP_PPP_2015'])
...
plt.title('Relationship Between Economic Wealth and Model Accuracy (2015)', fontsize=14)
plt.xlabel('Log10(GDP per capita, PPP)', fontsize=12)
plt.ylabel('Model Accuracy ($R^2$)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)

plt.tight_layout()
plt.savefig('gdp_vs_accuracy_log2015.png', dpi=300)
plt.show()

missing = merged[merged['GDP_PPP_2015'].isna()]['Holdout Country'].unique()
if len(missing) > 0:
